...
public class PhotonVisionSystem extends SubsystemBase {
  ...
  public PhotonVisionSystem() {
    try {
      m_photonCameraIntake = new PhotonCamera(CameraConstants.Intake.kCameraName);
      m_photonCameraShooter = new PhotonCamera(CameraConstants.ShooterBlue.kCameraName);
    } catch (Exception e) {
    ...
    m_robotToCamShooter = new Transform3d(
        new Translation3d(
            CameraConstants.ShooterBlue.kCameraPositionX,
            CameraConstants.ShooterBlue.kCameraPositonY,
            CameraConstants.ShooterBlue.kCameraPositionZ),
        new Rotation3d(
            CameraConstants.ShooterBlue.kCameraRoll,
            CameraConstants.ShooterBlue.kCameraPitch,
            CameraConstants.ShooterBlue.kCameraYaw));
  ...
  public double getAngleToTarget(Pose2d robotPose, int targetID) {
    ...
    if (Constants.isLogging) {
      Logger.recordOutput("Angle/Raw", angleToTarget);
      Logger.recordOutput("Angle/Offset", offset);
      Logger.recordOutput("Angle/Calculated", calculated);
    }
    return calculated;
  }

  public double getAngleToPose(Pose2d robotPose, Translation2d target) {
    double angleToTarget = Math.atan2(target.getY() - robotPose.getY(), target.getX() - robotPose.getX());
  ...
  public double getDistanceToTarget(Pose2d robotPose, int targetID) {
    ...
    if (Constants.isLogging) {
      Logger.recordOutput("Distance/Raw", distanceRaw);
      Logger.recordOutput("Distance/Calculated", distanceRaw);
    }
  ...
  public double getAngleToNote() {
    ...
    if (result.hasTargets()) {
      ...
      if (Constants.isLogging) {
        Logger.recordOutput("AngleToNote", yaw);
      }
